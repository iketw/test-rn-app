...
var jsonrpcTypes = require('@walletconnect/jsonrpc-types');
var jsonrpcUtils = require('@walletconnect/jsonrpc-utils');
var WalletConnect = require('@walletconnect/legacy-client');
var QRCodeModal = require('@walletconnect/legacy-modal');
var EventEmitter = require('eventemitter3');
var jsonrpcHttpConnection = require('@walletconnect/jsonrpc-http-connection');
var jsonrpcProvider = require('@walletconnect/jsonrpc-provider');
var legacyTypes = require('@walletconnect/legacy-types');
...
var QRCodeModal__default = /*#__PURE__*/_interopDefault(QRCodeModal);
var EventEmitter__default = /*#__PURE__*/_interopDefault(EventEmitter);

class SignerConnection extends jsonrpcTypes.IJsonRpcConnection {
  constructor(opts) {
    super();
    defineProperty._defineProperty(this, "events", new EventEmitter__default["default"]());
    defineProperty._defineProperty(this, "accounts", []);
    defineProperty._defineProperty(this, "chainId", 1);
    defineProperty._defineProperty(this, "pending", false);
    defineProperty._defineProperty(this, "wc", void 0);
    defineProperty._defineProperty(this, "bridge", "https://bridge.walletconnect.org");
    defineProperty._defineProperty(this, "qrcode", true);
    defineProperty._defineProperty(this, "qrcodeModalOptions", undefined);
    defineProperty._defineProperty(this, "opts", void 0);
    this.opts = opts;
  ...
  async send(payload) {
    ...
    this.sendPayload(payload).then(res => this.events.emit("payload", res)).catch(e => this.events.emit("payload", jsonrpcUtils.formatJsonRpcError(payload.id, e.message)));
  }
  ...
  register(opts) {
    ...
    const connectorOpts = {
      bridge: this.bridge,
      qrcodeModal: this.qrcode ? QRCodeModal__default["default"] : undefined,
    ...
    this.wc = typeof opts?.connector !== "undefined" ? opts.connector : new WalletConnect__default["default"](connectorOpts);
    if (typeof this.wc === "undefined") {
  ...
  sanitizeResponse(response) {
    return typeof response.error !== "undefined" && typeof response.error.code === "undefined" ? jsonrpcUtils.formatJsonRpcError(response.id, response.error.message) : response;
  }
}
...
class WalletConnectProvider {
  constructor(opts) {
    defineProperty._defineProperty(this, "events", new EventEmitter__default["default"]());
    defineProperty._defineProperty(this, "rpc", void 0);
    defineProperty._defineProperty(this, "signer", void 0);
    defineProperty._defineProperty(this, "http", void 0);
    this.rpc = {
      infuraId: opts?.infuraId,
      custom: opts?.rpc
    };
    this.signer = new jsonrpcProvider.JsonRpcProvider(new SignerConnection$1(opts));
  ...
  async request(args) {
    ...
    if (legacyTypes.SIGNING_METHODS.includes(args.method)) {
  ...
  setHttpProvider(chainId) {
    const rpcUrl = legacyUtils.getRpcUrl(chainId, this.rpc);
    if (typeof rpcUrl === "undefined") {
      return undefined;
    }
    const http = new jsonrpcProvider.JsonRpcProvider(new jsonrpcHttpConnection.HttpConnection(rpcUrl));
    return http;
  }
}
